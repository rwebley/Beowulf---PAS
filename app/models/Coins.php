...
class Coins extends Zend_Db_Table_Abstract {
	...
	*/
	public function getCoinData($id) {
		...
		$select = $coins->select()
						...
						->joinLeft('emperors','emperors.pasID = rulers.id',array('empid' => 'id'))
	...
	*/
	public function getCoinToEdit($id) {
		$coins = $this->getAdapter();
		$select = $coins->select()
						->from($this->_name, array('ruler' => 'ruler_id', 'ruler_qualifier', 'denomination',
