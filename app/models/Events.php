...
class Events extends Pas_Db_Table_Abstract {
	...
	*/
	...
	public function getEventsDate($day,$params) {
		...
		$select = $events->select()
                       ->from($this->_name)
   					   ->joinLeft('staffregions','events.eventRegion = staffregions.ID',array('rID' => 'ID','description'))
	...
	*/
	...
	public function getAttendanceAdults($datefrom = NULL,$dateto = NULL){
		$select = $this->select()
                       ->from($this->_name,array('q' => 'SUM(adultsAttend)',
	...
	*/
	...
	public function getMapData() {
		$events = $this->getAdapter();
		$select = $events->select()
                      ->from($this->_name, array( 'id','name' => 'eventTitle',
